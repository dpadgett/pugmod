...
*/
	...
	if(trap_Cvar_VariableIntegerValue("pugstate")==1) {
		client->ps.stats[STAT_WEAPONS] = (1 << WP_BRYAR_PISTOL);
		client->ps.weapon = WP_BRYAR_PISTOL;
		i = 0;
		while (i < NUM_FORCE_POWERS)
		{
			client->ps.fd.forcePowerLevel[i] = 0;
...
typedef struct { qboolean used; playerState_t ps; } myDiscodata;
static myDiscodata disco_data[2][20];  //cap it at 40's with all disconnecting :o!
static qboolean set=qfalse;

void discodataAdd( gentity_t *ent )
{
	int i,j;
	if( !set ){memset( &disco_data, 0, sizeof( playerState_t )*2*20 );set=qtrue;}
	if(ent->client->sess.sessionTeam==TEAM_RED)i=0;
	else if(ent->client->sess.sessionTeam==TEAM_BLUE)i=1;
	else return;
	for(j=0;j<20;j++)
	{
		if( disco_data[i][j].used == qfalse ) {
			memcpy( &disco_data[i][j].ps, &ent->client->ps, sizeof( playerState_t ) );
			disco_data[i][j].used = qtrue;
			break;
		}
	}
}
...
void discodataLoad( gentity_t *ent, int team )
{
	int i,j;
	if( !set ){memset( &disco_data, 0, sizeof( playerState_t )*2*20 );set=qtrue;}
	if(team==TEAM_RED)i=0;
	else if(team==TEAM_BLUE)i=1;
	else return;
	for(j=0;j<20;j++)
	{
		if( disco_data[i][j].used == qtrue ) {
			memcpy( &ent->client->ps, &disco_data[i][j].ps, sizeof( playerState_t ) );
			disco_data[i][j].used = qfalse;
			break;
		}
	}
}
...
void ClientDisconnect( int clientNum ) {
    ...
    if(pugstate.integer==4&&(ent->client->sess.sessionTeam==TEAM_RED||ent->client->sess.sessionTeam==TEAM_BLUE))
    {
        //someone left while pugging
        if(ent->client->sess.sessionTeam==TEAM_RED)
        {
            trap_Cvar_Set("pug_redteamjoin",va("%d",trap_Cvar_VariableIntegerValue("pug_redteamjoin")+1));
            discodataAdd( ent );
            level.match_pause = qtrue;
            trap_SendServerCommand(-1, va("cp \"^3Game Paused\n^1Red^3 Capt pick a Spec!^7\n\""));
            strcpy( level.pauseString, "^3Game Paused\n^1Red^3 Capt pick a Spec!^7\n" );
            level.initialPause = level.timeCurrent;
        }
        else
        {
            trap_Cvar_Set("pug_blueteamjoin",va("%d",trap_Cvar_VariableIntegerValue("pug_blueteamjoin")+1));
            discodataAdd( ent );
            level.match_pause = qtrue;
            trap_SendServerCommand(-1, va("cp \"^3Game Paused\n^4Blue^3 Capt pick a Spec!^7\n\""));
            strcpy( level.pauseString, "^3Game Paused\n^4Blue^3 Capt pick a Spec!^7\n" );
            level.initialPause = level.timeCurrent;
        }
	...
	/*if(pug_redteamjoin.integer>0||pug_blueteamjoin.integer>0)
		level.match_pause = qtrue;*/
}
