...
void ClientTimerActions(gentity_t*,int);
...
void SpectatorThink( gentity_t *ent, usercmd_t *ucmd ) {
	...
	if(!level.match_pause||pug_pickedclient.integer==ent-g_entities)
		ClientTimerActions( ent, ucmd->serverTime - client->ps.commandTime );
	else {
		while(level.timeCurrent - level.initialPause>=1000) {
			trap_SendServerCommand( -1, va("cp \"%s\"",level.pauseString));
			level.initialPause += 1000;
		}
		G_Printf("%d (%d)\n",level.timeCurrent,level.initialPause);
	}
}
...
void ClientTimerActions( gentity_t *ent, int msec ) {
	gclient_t	*client;
	static int pugpicktime = 0;
	static int initialTime = 0; //meh, specs r weird...

	client = ent->client;
	client->timeResidual += msec;

	if( pug_pickedclient.integer == -1 && pugpicktime ) pugpicktime = 0;
	else if ( pug_pickedclient.integer == ent - g_entities ) {
		if ( pugpicktime == 0 ) initialTime = level.timeCurrent - 1000;
		while( level.timeCurrent - initialTime >= 1000 )
		{
			initialTime += 1000;
				if( --pugpicktime == -1 ) {
					pugpicktime = 15; //15 second timeout
					trap_SendServerCommand( ent - g_entities, va( "cp \"^2You've been picked!^7\nVote Yes to join the %s team!",pugstate.integer==2||pug_blueteamjoin.integer>0?"^4Blue^7":"^1Red^7") );
				} else if( pugpicktime == 0 ) { //out of time
					trap_SendServerCommand( -1, va( "chat \"^7%s^7 is afk or spec, pick again.\"", ent->client->pers.netname ));
					trap_Cvar_Set( "pug_pickedclient", "-1" );
					pug_pickedclient.integer = -1;
				} else trap_SendServerCommand( ent - g_entities, va( "cp \"^2You've been picked!^7\nVote Yes to join the %s team!",pugstate.integer==2||pug_blueteamjoin.integer>0?"^4Blue^7":"^1Red^7") );
		}
	}
...
//	WP_SaberPositionUpdate(ent, ucmd); //check the server-side saber point, do apprioriate server-side actions (effects are cs-only)
	...
	*/
	...
	if (pm.cmd.generic_cmd && !level.match_pause &&
		(pm.cmd.generic_cmd != ent->client->lastGenCmd || ent->client->lastGenCmdTime < level.timeCurrent))
	{
		...
		{ //these are the only two where you wouldn't care about a delay between
			ent->client->lastGenCmdTime = level.timeCurrent + 300; //default 100ms debounce between issuing the same command.
		}
...
//	G_VehicleAttachDroidUnit( ent );
	...
	if (client->ps.forceKickFlip)
	{
		...
		if (faceKicked && faceKicked->client && !level.match_pause && (!OnSameTeam(ent, faceKicked) || g_friendlyFire.integer) &&
...
//			ent->client->timeoutTime += time_delta;
			ent->client->pers.enterTime += time_delta;
			ent->client->pers.netnameTime += time_delta;
...
//			ent->client->sniperRifleFiredTime += time_delta;
			...
			ent->fly_sound_debounce_time += time_delta;
//			ent->voiceChatPreviousTime += time_delta;
//			ent->ChatPreviousTime += time_delta;
			ent->client->lastGenCmdTime += time_delta;
		}
