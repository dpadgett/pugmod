...
void discodataLoad( gentity_t *ent, int team );
...
void SetTeam( gentity_t *ent, char *s ) {
    if(pugstate.integer>0)
    {
        if(!Q_strncmp(s,"pug",3))s+=3;
        else
        {
            if(ent->client->sess.sessionTeam==TEAM_SPECTATOR&&
               (s[0]=='s'||s[0]=='f'))
		...
		if(s[0]=='r'&&pug_redteamjoin.integer>0)
		{
			if(pug_redteamjoin.integer-1==0&&pug_blueteamjoin.integer==0)level.match_pause=qfalse;
			trap_Cvar_Set("pug_redteamjoin",va("%d",pug_redteamjoin.integer-1));
			if(pug_redteamjoin.integer==0&&pug_blueteamjoin.integer==0)level.match_pause=qfalse;
			trap_Cvar_Set("pugstate","4");
		}
		else if(s[0]=='b'&&pug_blueteamjoin.integer>0)
		{
			if(pug_redteamjoin.integer==0&&pug_blueteamjoin.integer-1==0)level.match_pause=qfalse;
			trap_Cvar_Set("pug_blueteamjoin",va("%d",pug_blueteamjoin.integer-1));
			trap_Cvar_Set("pugstate","4");
        }
	}
	int					team, oldTeam;
...
void G_Say( gentity_t *ent, gentity_t *target, int mode, const char *chatText ) {
	...
	if( Q_stricmp( chatText, "^1JAM^7's ^1JK2^7/^1JK3 ^7Kill Tracker v^12.0^7 - ^1jamtracker.tk^7" ) == 0 ||
        Q_stricmp( chatText, "^7Silence ^2deactivated - messages will now be shown" ) == 0 )
	{
        G_Say( ent, target, mode, "!silence" );
    }
}
...
//	char*	type = NULL;
	...
	} else if ( !Q_stricmp( arg1, "pugcapt" ) ) {
	} else {
		trap_SendServerCommand( ent-g_entities, "print \"Invalid vote string.\n\"" );
	...
	else if ( !Q_stricmp( arg1, "pugcapt" ) )
	{
		...
		if ( clientid == -1 )
		{
			...
			if (clientid == -1 && *arg2!='#' )
			{
				...
				Com_sprintf( arg2, 10, "#%i", level.sortedClients[atoi(arg2+1)]);
		}
        clientid = G_ClientNumberFromName ( arg3 );

		if ( clientid == -1 )
		{
			clientid = G_ClientNumberFromStrippedName(arg3);
			...
			if (clientid == -1 && *arg3!='#')
			{
				...
				Com_sprintf( arg3, 10, "#%i", level.sortedClients[atoi(arg3+1)]);
		}
		Com_sprintf( level.voteString, sizeof( level.voteString ), "%s \'%s\' \'%s\'", arg1, arg2, arg3 );
		Com_sprintf( level.voteDisplayString, sizeof( level.voteDisplayString ), "%s", level.voteString );
    }
	else
	{
...
void Cmd_Vote_f( gentity_t *ent ) {
	...
	if ( pug_pickedclient.integer == ent - g_entities ) {
		trap_Cvar_Set( "pug_pickedclient", "-1" );
		if ( msg[0] == 'y' || msg[1] == 'Y' || msg[1] == '1' ) { //voted YES
			if ( pugstate.integer == 2 ) {
				SetTeam(ent,"pugb");
				trap_Cvar_Set("pugstate","3");
			} else if( pug_blueteamjoin.integer>0 ) {
				SetTeam(ent,"pugb");
				discodataLoad( ent, TEAM_RED );
			} else if( pugstate.integer == 3) {
				SetTeam(ent,"pugr");
				trap_Cvar_Set("pugstate","2");
			} else if( pug_redteamjoin.integer>0 ) {
				SetTeam(ent,"pugr");
				discodataLoad( ent, TEAM_BLUE );
			}
		} else //otherwise voted NO
			trap_SendServerCommand( -1, va("chat \"^2%s^2 has declined your pick.  Pick again.^7\"",ent->client->pers.netname));
		return;
	}
...
void ClientCommand( int clientNum ) {
	...
	if (Q_stricmp (cmd, "say") == 0) {
        ...
        if( ( pugstate.integer==2 || pug_blueteamjoin.integer > 0 ) &&
			pug_pickedclient.integer==-1 &&
			ent-g_entities==trap_Cvar_VariableIntegerValue("pug_bluecapt") )
        {
            char pickedname[MAX_STRING_CHARS],pickname[MAX_STRING_CHARS],name[MAX_STRING_CHARS];
            trap_Argv(1,pickedname,sizeof(pickedname));
            SanitizeString(pickedname,pickname);
            int i;
            for(i=0;i<level.numConnectedClients;i++)
            {
                SanitizeString((&level.clients[ level.sortedClients[ i ] ])->pers.netname,name);
                if(Q_stricmp(name,pickname)==0||(pickname[0]=='#'&&atoi(pickname+1)==i))
                {
                    //got the player picked
                    if((&level.clients[ level.sortedClients[ i ] ])->sess.sessionTeam==TEAM_SPECTATOR)
                    {
                        //make sure they're spec
                        if(strstr(name,"spec")||strstr(name,"afk"))
                        {
                            trap_SendServerCommand( -1, va("chat \"^2That player is speccing or afk.  Pick again.^7\""));
                        }
                        else
                        {
                            trap_SendServerCommand( -1, va( "chat \"^2Picked player %s^7",level.clients[ level.sortedClients[ i ] ].pers.netname) );
                            trap_Cvar_Set("pug_pickedclient",va("%i",level.sortedClients[i]));
                            pug_pickedclient.integer = level.sortedClients[i];
                        }
                    }
                    else
                    {
                        trap_SendServerCommand( -1, va("chat \"^2That player is not a spectator.  Pick again.^7\""));
                    }
                }
            }
        }
        else if(( pugstate.integer==3 || pug_redteamjoin.integer > 0 ) &&
				pug_pickedclient.integer==-1 &&
				ent-g_entities==trap_Cvar_VariableIntegerValue("pug_redcapt") )
        {
            char pickedname[MAX_STRING_CHARS],pickname[MAX_STRING_CHARS],name[MAX_STRING_CHARS];
            trap_Argv(1,pickedname,sizeof(pickedname));
            SanitizeString(pickedname,pickname);
...
#endif
#if 0
    else if (Q_stricmp(cmd, "pugcapt") == 0)
    {
        //start capts - lock teams
        if(ent->client->sess.sessionTeam == TEAM_RED || ent->client->sess.sessionTeam == TEAM_BLUE )
        {
            //determine it's a 1v1 or else don't set it
            int i,red=0,blue=0;
            qboolean goodteams = qtrue;
            for(i=0;i<level.maxclients;i++)
            {
                if ( level.clients[ i ].pers.connected == CON_DISCONNECTED )
        			continue;
                //trap_SendServerCommand( clientNum, va("print \"# connected: %d  i:%d team:%d\n\"", level.numConnectedClients,i,(&level.clients[ level.sortedClients[ i ] ])->sess.sessionTeam,(&level.clients[ level.sortedClients[ i ] ])->ps.persistant[PERS_TEAM] ) );
                if(level.clients[ i ].sess.sessionTeam==TEAM_RED)
                {
                    red+=i+1;
                    if(red>i+1){goodteams=qfalse;break;}
                }
                else if(level.clients[ i ].sess.sessionTeam==TEAM_BLUE)
                {
                    blue+=i+1;
                    if(blue>i+1){goodteams=qfalse;break;}
                }
                /*else
                {
                    goodteams=qfalse;
            ...
            if(goodteams==qtrue&&red&&blue)
            {
                trap_Cvar_Set("pugstate","1"); //start capts
                trap_Cvar_Set("pug_redcapt",va("%d",red-1)); //set red capt index
                trap_Cvar_Set("pug_bluecapt",va("%d",blue-1)); //set blue capt index
                trap_Cvar_Set("pug_bluecaptweap",va("%d",level.clients[ blue-1 ].ps.stats[STAT_WEAPONS]));
                trap_Cvar_Set("pug_redcaptweap",va("%d",level.clients[ red-1 ].ps.stats[STAT_WEAPONS]));
                level.clients[ blue-1 ].ps.stats[STAT_WEAPONS] = (1 << WP_BRYAR_PISTOL);
                level.clients[ blue-1 ].ps.weapon = WP_BRYAR_PISTOL;
                level.clients[ red-1 ].ps.stats[STAT_WEAPONS] = (1 << WP_BRYAR_PISTOL);
                level.clients[ red-1 ].ps.weapon = WP_BRYAR_PISTOL;
				i = 0;
				while (i < NUM_FORCE_POWERS)
				{
					level.clients[ red-1 ].ps.fd.forcePowerLevel[i] = 0;
					level.clients[ red-1 ].ps.fd.forcePowersKnown &= ~(1 << i);
					level.clients[ blue-1 ].ps.fd.forcePowerLevel[i] = 0;
					level.clients[ blue-1 ].ps.fd.forcePowersKnown &= ~(1 << i);
					i++;
				}
				level.clients[ red-1 ].ps.fd.forcePowerLevel[FP_LEVITATION] = 3;
				level.clients[ blue-1 ].ps.fd.forcePowerLevel[FP_LEVITATION] = 3;
				WP_SpawnInitForcePowers( g_entities+red-1 );
				WP_SpawnInitForcePowers( g_entities+blue-1 );
            }
            else trap_SendServerCommand( clientNum, va("print \"Not a 1v1 - I got %d on red and %d on blue.\n\"", red, blue ) );
        } else {
            trap_SendServerCommand( clientNum, va("print \"You must be on a team to force capts.\n\"" ) );
        }
    }
#endif
    else if (Q_stricmp(cmd, "pug_serverstatus") == 0)
    {
        int i;
        char tmpName[MAX_STRING_CHARS];
        for(i=0;i<level.numConnectedClients;i++)
        {
			SanitizeString2((&level.clients[ level.sortedClients[ i ] ])->pers.netname,tmpName);
            if(level.clients[level.sortedClients[ i ]].pers.connected!=CON_DISCONNECTED)trap_SendServerCommand( clientNum, va("print \"%d: %s^7 (%s)\n\"", i, (&level.clients[ level.sortedClients[ i ] ])->pers.netname, tmpName ) );
        }
    }
    else if (Q_stricmp(cmd, "pug_start") == 0)
    {
        if(trap_Cvar_VariableIntegerValue("pugstate")==2||trap_Cvar_VariableIntegerValue("pugstate")==3)
        {
            char tmpstr[MAX_STRING_CHARS];
            trap_Argv( 1, tmpstr, sizeof( tmpstr ) );
            if(Q_stricmp(tmpstr, "map_restart") == 0)
            {
                int oldvote;
                oldvote = g_allowVote.integer;
                g_allowVote.integer = 1;
                Cmd_CallVote_f( g_entities + clientNum );
                g_allowVote.integer = oldvote;
            }
        }
    }
	else
