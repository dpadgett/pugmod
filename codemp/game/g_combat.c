...
void player_die( gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int meansOfDeath ) {
    ...
    if(trap_Cvar_VariableIntegerValue("pugstate")==1) //capting, should be 1v1
    {
        if(meansOfDeath==MOD_BRYAR_PISTOL||meansOfDeath==MOD_BRYAR_PISTOL_ALT||meansOfDeath==MOD_FALLING)
        {
            // 2 = blue picks,
            // 3 = red picks
            //trap_SendServerCommand( attacker - g_entities, va("print \"client # %d.\n\"", attacker - g_entities ) );
            if((attacker->client-level.clients)==trap_Cvar_VariableIntegerValue("pug_redcapt"))
                trap_Cvar_Set("pugstate","3");
            else
                trap_Cvar_Set("pugstate","2");
            if(meansOfDeath==MOD_FALLING)
                trap_Cvar_Set("pugstate",attacker->client-level.clients==pug_redcapt.integer?"2":"3");
            level.clients[ trap_Cvar_VariableIntegerValue("pug_bluecapt") ].ps.stats[STAT_WEAPONS] = trap_Cvar_VariableIntegerValue("pug_bluecaptweap");
            level.clients[ trap_Cvar_VariableIntegerValue("pug_redcapt") ].ps.stats[STAT_WEAPONS] = trap_Cvar_VariableIntegerValue("pug_redcaptweap");
			WP_InitForcePowers( g_entities+trap_Cvar_VariableIntegerValue("pug_redcapt") );
			WP_InitForcePowers( g_entities+trap_Cvar_VariableIntegerValue("pug_bluecapt") );
        }
        else
        {
            trap_SendServerCommand( attacker - g_entities, va("print \"n1.  Try using a pistol next time Sherlock.\n\"" ) );
        }
        // MOD_BRYAR_PISTOL || MOD_BRYAR_PISTOL_ALT are only legal means of killing for capts
    }
	...
	*/
	{
		...
		{ //Some droids don't have death anims
			...
			if (1/*meansOfDeath == MOD_SABER || (meansOfDeath == MOD_MELEE && G_HeavyMelee( attacker )) */)//saber or heavy melee (claws)
...
void G_Damage( gentity_t *targ, gentity_t *inflictor, gentity_t *attacker,
    ...
    if (trap_Cvar_VariableIntegerValue("pugstate")==1&&mod!=MOD_BRYAR_PISTOL&&mod!=MOD_BRYAR_PISTOL_ALT&&mod!=MOD_FALLING)
        return;
