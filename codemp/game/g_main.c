...
vmCvar_t        pugstate;
vmCvar_t        pug_redcapt;
vmCvar_t        pug_bluecapt;
vmCvar_t        pug_redteamjoin;
vmCvar_t        pug_blueteamjoin;
vmCvar_t        pug_pickedclient;
...
// DUEL_HEALTH
	...
	{ &pugstate, "pugstate", "0", CVAR_INTERNAL | CVAR_NORESTART | CVAR_ROM, 0, qtrue  },
	{ &pug_redcapt, "pug_redcapt", "0", CVAR_INTERNAL | CVAR_NORESTART | CVAR_ROM, 0, qtrue  },
	{ &pug_bluecapt, "pug_bluecapt", "0", CVAR_INTERNAL | CVAR_NORESTART | CVAR_ROM, 0, qtrue  },
	{ &pug_redteamjoin, "pug_redteamjoin", "0", CVAR_INTERNAL | CVAR_ROM, 0, qtrue  },
	{ &pug_blueteamjoin, "pug_blueteamjoin", "0", CVAR_INTERNAL | CVAR_ROM, 0, qtrue  },
	{ &gsess, "gsess", "", CVAR_INTERNAL | CVAR_ROM | CVAR_NORESTART, 0, qtrue  },
	{ &pug_pickedclient, "pug_pickedclient", "-1", CVAR_INTERNAL | CVAR_ROM, 0, qtrue  },
};
...
// nmckenzie: DUEL_HEALTH: Default.
	...
	else if(trap_Cvar_VariableIntegerValue("pugstate")==4)
	   trap_Cvar_Set("pugstate","0"); //let ppl join teams + fool around until next PUG
}
	...
	*/
	...
	if (g_gametype.integer != GT_SIEGE)
	{
		if ( g_timelimit.integer && !level.warmupTime ) {
			if ( level.timeCurrent - level.startTime >= g_timelimit.integer*60000 ) {
...
void CheckVote( void ) {
	if ( level.voteExecuteTime && level.voteExecuteTime < level.time ) {
		...
		if( !Q_stricmpn( level.voteString, "pugcapt", 7 ) )
		{
            char *arg2,*arg3;
            arg2 = level.voteString+9;
            int i;
            ...
            int clientid = G_ClientNumberFromName ( arg2 ),clientid2 = G_ClientNumberFromName ( arg3 );
    		if ( clientid != -1 || (clientid = G_ClientNumberFromStrippedName(arg2)) != -1 || *arg2=='#' )
            {
                if( clientid2 != -1 || (clientid2 = G_ClientNumberFromStrippedName(arg3)) != -1 || *arg3=='#' )
                {
					if(*arg2=='#')clientid=atoi(arg2+1);
					if(*arg3=='#')clientid2=atoi(arg3+1);
                    for(i=0;i<level.numConnectedClients;i++)
                        if(level.sortedClients[ i ]!=clientid && level.sortedClients[ i ]!=clientid2)SetTeam(g_entities + level.sortedClients[ i ], pugstate.integer>0?"pugs":"s");
                    SetTeam(g_entities + clientid,pugstate.integer>0?"pugb":"b");
                    SetTeam(g_entities + clientid2,pugstate.integer>0?"pugr":"r");
                    trap_Cvar_Set("pug_redcapt",va("%d",clientid2));
                    trap_Cvar_Set("pug_bluecapt",va("%d",clientid));
                    trap_Cvar_Set("pugstate","1");
	                trap_Cvar_Set("pug_bluecaptweap",va("%d",level.clients[ clientid ].ps.stats[STAT_WEAPONS]));
	                trap_Cvar_Set("pug_redcaptweap",va("%d",level.clients[ clientid2 ].ps.stats[STAT_WEAPONS]));
	                level.clients[ clientid ].ps.stats[STAT_WEAPONS] = (1 << WP_BRYAR_PISTOL);
	                level.clients[ clientid ].ps.weapon = WP_BRYAR_PISTOL;
	                level.clients[ clientid2 ].ps.stats[STAT_WEAPONS] = (1 << WP_BRYAR_PISTOL);
	                level.clients[ clientid2 ].ps.weapon = WP_BRYAR_PISTOL;
					i = 0;
					while (i < NUM_FORCE_POWERS)
					{
						level.clients[ clientid2 ].ps.fd.forcePowerLevel[i] = 0;
						level.clients[ clientid2 ].ps.fd.forcePowersKnown &= ~(1 << i);
						level.clients[ clientid ].ps.fd.forcePowerLevel[i] = 0;
						level.clients[ clientid ].ps.fd.forcePowersKnown &= ~(1 << i);
						i++;
					}
					level.clients[ clientid2 ].ps.fd.forcePowerLevel[FP_LEVITATION] = 3;
					level.clients[ clientid ].ps.fd.forcePowerLevel[FP_LEVITATION] = 3;
					WP_SpawnInitForcePowers( g_entities+clientid2 );
					WP_SpawnInitForcePowers( g_entities+clientid );
					g_entities[clientid2].health = level.clients[ clientid2 ].ps.stats[STAT_MAX_HEALTH];
					g_entities[clientid].health = level.clients[ clientid ].ps.stats[STAT_MAX_HEALTH];
					level.clients[ clientid2 ].ps.stats[STAT_ARMOR] = 25;
					level.clients[ clientid ].ps.stats[STAT_ARMOR] = 25;
                    //trap_SendConsoleCommand( EXEC_APPEND, "map_restart" ); //restart cause for some reason team chaging doesn't always update properly =/
                } else {
                    for(i=0;i<level.numConnectedClients;i++)
           				trap_SendServerCommand( level.sortedClients[ i ], va("print \"There is no client named '%s' currently on the server. \\disco maybe?\n\"", arg3 ) );
                }
    		} else {
                for(i=0;i<level.numConnectedClients;i++)
        			trap_SendServerCommand( level.sortedClients[ i ], va("print \"There is no client named '%s' currently on the server. \\disco maybe?\n\"", arg2 ) );                
...
void G_RunThink (gentity_t *ent) {
	int	thinktime;// jaybird's fix for possible floating point calculation problem that can lead to crashing

	// ensiform - If paused, push nextthink
	if(level.match_pause && (ent - g_entities) >= g_maxclients.integer &&
...
void G_RunFrame( int levelTime ) {
	...
	if( !level.match_pause ) {
		...
		//level.jk2pp_levelTime = levelTime - level.jk2pp_timeDelta;
	} else {
		level.timeDelta = levelTime - level.timeCurrent;
		//level.jk2pp_timeDelta = levelTime - level.jk2pp_levelTime;
		if((level.time % 500) == 0) {
			// FIXME: set a PAUSE cs and let the client adjust their local starttimes
			//        instead of this spam
			//trap_SetConfigstring(CS_LEVEL_START_TIME, va("%i", level.startTime + level.jk2pp_timeDelta));
