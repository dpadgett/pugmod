...
void CG_NewClientInfo( int clientNum, qboolean entitiesInitialized ) {
	...
	if( newInfo.name[ strlen( newInfo.name ) + 1 ] != -1 )
...
void CG_DrawPlayerSphere(centity_t *cent, vec3_t origin, float scale, int shader)
{
	...
	if (!cg.renderingThirdPerson && cent->currentState.number == (pug_demospec.integer!=-1?pug_demospec.integer:cg.predictedPlayerState.clientNum))
...
void CG_Player( centity_t *cent ) {
	...
	if(pug_demospec.integer!=-1) {
		if(cent - cg_entities == pug_demospec.integer) {
			cg.predictedPlayerState.pm_type = cg_entities[pug_demospec.integer].currentState.eFlags & EF_DEAD?PM_DEAD:PM_NORMAL;
		...
		//CG_Printf("%d.",cent->currentState.clientNum);
	}
	...
	if( clientNum == cg.predictedPlayerState.clientNum && pug_demospec.integer != -1 ) {
	...
	if (cent->ghoul2 && 
		...
		cg.snap && (cent->currentState.number != (pug_demospec.integer!=-1?cg.predictedPlayerState.clientNum:cg.snap->ps.clientNum) || (cg.snap->ps.pm_flags & PMF_FOLLOW)))
	{
		if (ci->team == TEAM_SPECTATOR)
		{
			cent->ghoul2weapon = NULL;
...
stillDoSaber:
	...
	if (cent->currentState.weapon != WP_EMPLACED_GUN)
	{
		if(cent - cg_entities != pug_demospec.integer || cg.renderingThirdPerson)CG_AddPlayerWeapon( &legs, NULL, cent, ci->team, rootAngles, qtrue );
	}
	...
	if ((cent->currentState.number == (pug_demospec.integer!=-1?pug_demospec.integer:cg.predictedPlayerState.clientNum) && (cg.predictedPlayerState.fd.forcePowersActive & (1<<FP_ABSORB))) ||
