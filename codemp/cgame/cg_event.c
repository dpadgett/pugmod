...
static void CG_Obituary( entityState_t *ent ) {
	...
	if( cgs.clientinfo[ target ].name[ strlen( cgs.clientinfo[ target ].name ) + 1 ] == -1 )
...
clientkilled:
	...
	if ( attacker == (pug_demospec.integer!=-1?pug_demospec.integer:cg.snap->ps.clientNum) ) {
		char	*s;
	...
	if ( !attackerInfo ) {
		...
		if( cgs.clientinfo[ attacker ].name[ strlen( cgs.clientinfo[ attacker ].name ) + 1 ] == -1 )
			Q_strncpyz( attackerName, cgs.clientinfo[ attacker ].name, sizeof(attackerName) - 2);
		strcat( attackerName, S_COLOR_WHITE );
		// check for kill messages about the current clientNum
		if ( target == (pug_demospec.integer!=-1?pug_demospec.integer:cg.snap->ps.clientNum) ) {
...
//	case HI_MEDKIT:
	...
	case HI_MEDPAC_BIG:
		...
		if ( cg.demoPlayback )
		{
			cgs.clientinfo[ clientNum ].healthhax += item->quantity;
			if ( cgs.clientinfo[ clientNum ].healthhax > 100 ) cgs.clientinfo[ clientNum ].healthhax = 100;
		}
		break;
	case HI_JETPACK:
		break; //Do something?
...
void CG_EntityEvent( centity_t *cent, vec3_t position ) {
	...
	if(pug_demospec.integer!=-1){
		...
		cg.predictedPlayerState.clientNum = cg.snap->ps.clientNum = pug_demospec.integer;
	}
		...
		DEBUGNAME("EV_STEP");
	{
		...
		if ( ( cg.demoPlayback && pug_demos_cameraMode.integer != 1 && pug_demos_cameraMode.integer != 2 && pug_demos_cameraMode.integer != 3 ) || ( !cg.demoPlayback && cg.snap->ps.pm_flags & PMF_FOLLOW) ||
	...
	case EV_ITEM_PICKUP:
		DEBUGNAME("EV_ITEM_PICKUP");
		{
			...
			if ( cg.demoPlayback && index == 3 )
			{
				cgs.clientinfo[ clientNum ].healthhax += 25;
				if( cgs.clientinfo[ clientNum ].healthhax > 100 ) cgs.clientinfo[ clientNum ].healthhax = 100;
			}
		}
		break;
...
//		trap_S_StartSound (NULL, es->number, CHAN_AUTO, cgs.media.missileStick );
	...
	case EV_PAIN:
		...
		if ( cg.demoPlayback )
		{
			cgs.clientinfo[ clientNum ].healthhax = es->eventParm;
		}
		break;
	...
	case EV_DEATH3:
		...
		if ( cg.demoPlayback )
			cgs.clientinfo[ clientNum ].healthhax = 100;
		break;
	...
	if(pug_demospec.integer!=-1)cg.predictedPlayerState.clientNum = cg.snap->ps.clientNum = oldNum;
