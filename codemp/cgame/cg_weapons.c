...
void CG_AddViewWeapon( playerState_t *ps ) {
	...
	if ( !cg_drawGun.integer || ps->zoomMode) {
		vec3_t		origin;
	...
	CG_AddPlayerWeapon( &hand, ps, &cg_entities[ps->clientNum], ps->persistant[PERS_TEAM], angles, qfalse );
}
...
static qboolean isValidEnt( int number ) {
	if(cg_entities[number].currentState.eType == ET_MISSILE || cg_entities[number].currentState.eType == ET_PLAYER)return qtrue;
	else return qfalse;
}
...
void CG_NextWeapon_f( void ) {
	...
	if ( cg.demoPlayback ) {
		int oldidx, firstidx=-1;
		if(pug_demospec.integer==-1)pug_demospec.integer=cg.snap->ps.clientNum;
		oldidx = pug_demospec.integer;
		for( i=MAX_GENTITIES-1; i>=0; i-- )
		{
	    	//configstring = CG_ConfigString( clientNum + CS_PLAYERS );
		    if( /*( cg_entities[i].lerpOrigin[0] || cg_entities[i].lerpOrigin[1] || cg_entities[i].lerpOrigin[2] ) && cgs.clientinfo[i].team != TEAM_SPECTATOR*/
			isValidEnt( i ) ) {
				if( i>oldidx )pug_demospec.integer = i;
				firstidx = i;
			}
	    }
	    if( pug_demospec.integer == oldidx && firstidx != -1 )pug_demospec.integer = firstidx;
	    if(pug_demospec.integer==cg.snap->ps.clientNum)pug_demospec.integer=-1;
	    return;
	}
...
void CG_PrevWeapon_f( void ) {
	...
	if ( cg.demoPlayback ) {
	    ...
	    if( pug_demospec.integer == oldidx && lastidx != -1 )pug_demospec.integer = lastidx;
	    if(pug_demospec.integer==cg.snap->ps.clientNum)pug_demospec.integer=-1;
	    return;
	}
