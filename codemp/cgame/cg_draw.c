...
//	int val[5];
	...
	if( ui_noScope.integer || pug_demos_cameraMode.integer > 2 )return;
...
static float CG_DrawFPS( float y ) {
	...
	static unsigned short SV_previousTimes[FPS_FRAMES];
	static unsigned short SV_index;
	static int	SV_previous, SV_lastupdate;
	...
	t = cg.snap->serverTime;
	if( t != SV_previous )
	{
		frameTime = t - SV_previous;
		SV_previous = t;
		if (t - SV_lastupdate > 50)	//don't sample faster than this
		{
			SV_lastupdate = t;
			SV_previousTimes[SV_index % FPS_FRAMES] = frameTime;
			SV_index++;
		}
		// average multiple frames together to smooth changes out a bit
		total = 0;
		for ( i = 0 ; i < FPS_FRAMES ; i++ ) {
			total += SV_previousTimes[i];
		}
		if ( !total ) {
			total = 1;
		}
		SV_fps = 1000 * FPS_FRAMES / total;
	}

	s = va( "%ifps", fps );
	if( cg_drawSvFPS.integer ) s = va( "%ifps (sv: %i)", fps, SV_fps );
	w = CG_DrawStrlen( s ) * BIGCHAR_WIDTH;
...
static float CG_DrawTimer( float y ) {
	...
	s = va( "%i:%02i.%03i", mins, seconds, msec );
	w = CG_DrawStrlen( s ) * BIGCHAR_WIDTH;
...
static void CG_DrawPowerupIcons(int y)
{
	...
	for (j = 0; j <= PW_NUM_POWERUPS; j++)
	{
		if ((pug_demospec.integer!=-1?cg_entities[pug_demospec.integer].currentState.powerups & (1 << j):cg.snap->ps.powerups[j] > cg.time))
		{
...
static void CG_ScanForCrosshairEntity( void ) {
	...
	if(pug_demospec.integer!=-1 && cg.demoPlayback) {
		oldNum = cg.snap->ps.clientNum;
		cg.snap->ps.clientNum = cg.predictedPlayerState.clientNum = pug_demospec.integer;
		oldTeam = cg.predictedPlayerState.persistant[PERS_TEAM];
		cg.predictedPlayerState.persistant[PERS_TEAM] = cg.snap->ps.persistant[PERS_TEAM] = cgs.clientinfo[pug_demospec.integer].team;
		VectorCopy(cg.predictedPlayerState.viewangles,orgview);
		VectorCopy(cg_entities[pug_demospec.integer].lerpAngles,cg.predictedPlayerState.viewangles);
	}
...
static void CG_DrawCrosshairNames( void ) {
	...
	if (cgs.gametype >= GT_TEAM)
	{
		...
		{ //instead of team-based we'll make it oriented based on which team we're on
			if (cgs.clientinfo[cg.crosshairClientNum].team == (pug_demospec.integer!=-1?cgs.clientinfo[pug_demospec.integer].team:cg.predictedPlayerState.persistant[PERS_TEAM]))
			{
				baseColor = CT_GREEN;
			}
			else
	...
	else
	{
		//baseColor = CT_WHITE;
		if (cgs.gametype == GT_POWERDUEL &&
			cgs.clientinfo[pug_demospec.integer!=-1?pug_demospec.integer:cg.snap->ps.clientNum].team != TEAM_SPECTATOR &&
	...
	tcolor[3] = pug_demospec.integer!=-1?1.0f:color[3]*0.5f;
...
//		trap_S_StartLocalSound( cgs.media.talkSound, CHAN_LOCAL_SOUND );
	...
	else if (strncmp(cgs.voteString, "pugcapt", 7)==0)
	{
		Q_strncpyz(sCmd, "Capt:", sizeof(sCmd));
		for(i=9;cgs.voteString[i]&&cgs.voteString[i]!='\'';i++)
		Q_strncpyz(sParm, cgs.voteString+9, sizeof(sParm));
		sParm[i-9]=0;
		strcat(sParm," vs ");
		strcat(sParm,cgs.voteString+i+3);
		sParm[strlen(sParm)-1]=0;
	}
...
static qboolean CG_DrawFollow( void ) 
{
	...
	if ( !(cg.snap->ps.pm_flags & PMF_FOLLOW) && pug_demospec.integer == -1 ) 
	{
		return qfalse;
	}
...
//	s = "following";
	...
	if(pug_demospec.integer!=-1)s = cgs.clientinfo[ pug_demospec.integer ].name;
	CG_Text_Paint ( 320 - CG_Text_Width ( s, 2.0f, FONT_MEDIUM ) / 2, 80, 2.0f, colorWhite, s, 0, 0, 0, FONT_MEDIUM );
	if( pug_demospec.integer!=-1) {
		int i;
		for( i = 0; i < cg.snap->numEntities; i++ )
			if( cg.snap->entities[i].clientNum == pug_demospec.integer) return qtrue;
		CG_Text_Paint ( 320 - CG_Text_Width ( "Out of Range!", 2.0f, FONT_MEDIUM ) / 2, 120, 2.0f, colorWhite, "Out of Range!", 0, 0, 0, FONT_MEDIUM );
	}
...
static void CG_DrawSiegeDeathTimer( int timeRemaining )
{
	...
	item = Menu_FindItemByName(menuHUD, "deathframe");
...
static void CG_Draw2D( void ) {
	...
	if ( ui_fragsOnly.integer != 0 ) {
		vec4_t hcolor = {0, 0, 0, 0};
		if(!cg.renderingThirdPerson&&pug_demos_cameraMode.integer==0)CG_DrawZoomMask();
		CG_DrawRect(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT, SCREEN_WIDTH*SCREEN_HEIGHT, hcolor);
		CG_DrawCenterString();
		return;
	}
...
*/
	...
	if (cg.snap->ps.fallingToDeath && pug_demos_cameraMode.integer == 0)
	{
		vec4_t	hcolor;
	...
	if ( cg_siegeDeathTime )
	{
		...
		{
			timeRemaining = 0;
			cg_siegeDeathTime += cgs.g_siegeRespawn * 1000;
		}
...
void CG_DrawActive( stereoFrame_t stereoView ) {
	float		separation;
	vec3_t		baseOrg;
	char		wireFrame[MAX_STRING_CHARS];
	static int	oldTri = 0;
	GLuint		box;
	static int	fakeShader = -1;
	
	if( fakeShader == -1 ) fakeShader = trap_R_RegisterShader( "<default>" );
	...
	trap_Cvar_VariableStringBuffer( "r_showtris", wireFrame, sizeof( wireFrame ) );
	if( atoi( wireFrame ) > 0 ) {
...
//		glDisableClientState (GL_TEXTURE_COORD_ARRAY);
	...
	//{
		...
		/*struct {
			float x;
			float y;
		} j8[8] = { {0.5625, 0.4375}, {0.0625, 0.9375}, {0.3125, 0.6875}, {0.6875, 0.8125}, {0.8125, 0.1875}, {0.9375, 0.5625}, {0.4375, 0.0625}, {0.1875, 0.3125} };
		refdef_t tmpRefDef;
		vec3_t dir2Me;
		static int jitter = 0;
		...
		jitter = ( jitter + 1 ) % 8;
	        ...
	        //VectorMA( cg.refdef.vieworg, /*-0.33**/j8[jitter].x, cg.refdef.viewaxis[1], tmpRefDef.vieworg );
	        //VectorMA( cg.refdef.vieworg, /*-0.33**/j8[jitter].y, cg.refdef.viewaxis[2], tmpRefDef.vieworg );
	        //VectorSubtract( /*atOrg*/cg_crosshairPos, tmpRefDef.vieworg, dir2Me );
			//vectoangles( dir2Me, tmpRefDef.viewangles );
	        trap_R_RenderScene( &cg.refdef );
	        //glCallList(box);
