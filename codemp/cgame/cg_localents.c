...
localEntity_t	*CG_AllocLocalEntity( void ) {
	...
	if ( !cg_freeLocalEntities ) {
		...
		CG_Printf("^1WARNING: CG_AllocLocalEntity: out of nodes^7\n");
	}
...
===================
*/
static void CG_AddSmoke( localEntity_t *le ) {
	refEntity_t	*re;
	float		perc;

	re = &le->refEntity;
	
	if( !le->data.rocketSmoke.life ) return;
	// fade / grow time
	perc = (float)( cg.time - le->startTime ) / le->data.rocketSmoke.life;

	re->shaderRGBA[0] = 0xff * ( le->data.rocketSmoke.rgb_s[0] * ( 1 - perc ) + le->data.rocketSmoke.rgb_e[0] * perc );
	re->shaderRGBA[1] = 0xff * ( le->data.rocketSmoke.rgb_s[1] * ( 1 - perc ) + le->data.rocketSmoke.rgb_e[1] * perc );
	re->shaderRGBA[2] = 0xff * ( le->data.rocketSmoke.rgb_s[2] * ( 1 - perc ) + le->data.rocketSmoke.rgb_e[2] * perc );
	re->shaderRGBA[3] = 0xff * ( le->data.rocketSmoke.alpha_s * ( 1 - perc ) + le->data.rocketSmoke.alpha_e * perc );
	re->radius = le->data.rocketSmoke.size_s * ( 1 - perc ) + le->data.rocketSmoke.size_e * perc;
...
For rocket flames that hang in place, fade out, and are
removed if the view passes through them.
There are often many of these, so it needs to be simple.
not LOL
===================
...
static void CG_AddTrail( localEntity_t *le ) {
	...
	float		len;
	polyVert_t	verts[4];
	vec3_t		origin, work, work2, work3;
	float		perc, d;
	
	if( !le->data.rocketTrail.life ) return;
	perc = (float)( cg.time - le->startTime ) / le->data.rocketTrail.life;
	
	// if the view would be "inside" the sprite, kill the sprite
	// so it doesn't add too much overdraw
	VectorSubtract( le->pos.trBase, cg.refdef.vieworg, delta );
	len = VectorLength( delta );
	if ( len < le->radius ) {
		CG_FreeLocalEntity( le );
		return;
	...
	VectorCopy( le->pos.trDelta, work );
	VectorNormalize( work );
	VectorScale( work, -1, work );
	VectorScale( work, le->data.rocketTrail.length_s * ( 1 - perc ) + le->data.rocketTrail.length_e * perc, work );
	
	VectorCopy( origin, verts[0].xyz );
	VectorAdd( origin, work, verts[1].xyz );
	VectorAdd( origin, work, verts[2].xyz );
	VectorCopy( origin, verts[3].xyz );
	
	// now expand :nervou
	
	VectorNormalize( work );
	VectorSubtract( cg.refdef.vieworg, origin, work2 );
	VectorNormalize( work2 );
	d = DotProduct( work2, work );
	VectorMA( work2, -d, work, work2 );
	VectorNormalize( work2 );
	CrossProduct( work2, work, work3 );
	VectorNormalize( work3 );
	VectorScale( work3, le->data.rocketTrail.size_s * ( 1 - perc ) + le->data.rocketTrail.size_e * perc, work3 );
	
	VectorAdd( work3, verts[0].xyz, verts[0].xyz );
	VectorAdd( work3, verts[1].xyz, verts[1].xyz );
	VectorSubtract( verts[2].xyz, work3, verts[2].xyz );
	VectorSubtract( verts[3].xyz, work3, verts[3].xyz );
	...
	if( le->data.rocketTrail.hshader == cgs.media.smokePuffShader )
	{
		perc = (float)( cg.time - le->startTime ) / 20;
		if( perc > 1 ) perc = 1;
	}
	verts[0].modulate[0] = verts[1].modulate[0] = verts[2].modulate[0] = verts[3].modulate[0] = 
		( le->data.rocketTrail.rgb_s[0] * ( 1 - perc ) + le->data.rocketTrail.rgb_e[0] * perc ) * 0xff;
	
	verts[0].modulate[1] = verts[1].modulate[1] = verts[2].modulate[1] = verts[3].modulate[1] = 
		( le->data.rocketTrail.rgb_s[1] * ( 1 - perc ) + le->data.rocketTrail.rgb_e[1] * perc ) * 0xff;
	
	verts[0].modulate[2] = verts[1].modulate[2] = verts[2].modulate[2] = verts[3].modulate[2] = 
		( le->data.rocketTrail.rgb_s[2] * ( 1 - perc ) + le->data.rocketTrail.rgb_e[2] * perc ) * 0xff;
	
	verts[0].modulate[3] = verts[1].modulate[3] = verts[2].modulate[3] = verts[3].modulate[3] = 
		( le->data.rocketTrail.alpha_s * ( 1 - perc ) + le->data.rocketTrail.alpha_e * perc ) * 0xff;
	
	trap_R_AddPolyToScene( le->data.rocketTrail.hshader, 4, verts );
}
...
void CG_AddLocalEntities( void ) {
	...
	for ( ; le != &cg_activeLocalEntities ; le = next ) {
		...
		if( pug_demos_HideLocalEntities.integer & ( 1 << le->leType ) ) continue;
		switch ( le->leType ) {
		default:
		...
		case LE_ROCKET_TRAIL:
			CG_AddTrail( le );
			break;
		
		case LE_ROCKET_SMOKE:
			CG_AddSmoke( le );
			break;
		}
	}
