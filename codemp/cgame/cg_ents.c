...
static void CG_EntityEffects( centity_t *cent ) {
	...
	// constant light glow
	if ( cent->currentState.constantLight && cent->currentState.eType != ET_PLAYER && cent->currentState.eType != ET_INVISIBLE && cent->currentState.eType != ET_BODY ) {
		int		cl;
		int		i, r, g, b;
...
void G_GetDismemberLoc(/*gentity_t*/centity_t *self, vec3_t boltPoint, int limbType)
{ //Just get the general area without using server-side ghoul2
	vec3_t fwd, right, up;

	AngleVectors(/*self->r.currentAngles*/self->lerpAngles, fwd, right, up);

	VectorCopy(/*self->r.currentOrigin*/self->lerpOrigin, boltPoint);
...
void G_GetDismemberBolt(/*gentity_t*/centity_t *self, vec3_t boltPoint, int limbType)
{
	int useBolt = /*self->genericValue5*/0;
	...
	VectorCopy(self->lerpOrigin/*client->ps.origin*/, properOrigin);
	VectorCopy(self->lerpAngles/*client->ps.viewangles*/, properAngles);

	//try to predict the origin based on velocity so it's more like what the client is seeing
	VectorCopy(self->currentState.pos.trDelta/*client->ps.velocity*/, addVel);
	...
	properAngles[0] = 0;
	properAngles[1] = self->lerpAngles/*client->ps.viewangles*/[YAW];
	properAngles[2] = 0;

	trap_G2API_GetBoltMatrix(self->ghoul2, 0, useBolt, &boltMatrix, properAngles, properOrigin, cg.time, NULL, self->modelScale);

	boltPoint[0] = boltMatrix.matrix[0][3];
	boltPoint[1] = boltMatrix.matrix[1][3];
	boltPoint[2] = boltMatrix.matrix[2][3];

	trap_G2API_GetBoltMatrix(self->ghoul2, 1, 0, &boltMatrix, properAngles, properOrigin, cg.time, NULL, self->modelScale);
	...
	{ //Make some saber hit sparks over the severed wrist area
		...
		te.currentState.eType = ET_EVENTS + EV_SABER_HIT;
		te.currentState.otherEntityNum = self->currentState.number;
		te.currentState.otherEntityNum2 = ENTITYNUM_NONE;
		te.currentState.weapon = 0;//saberNum
		te.currentState.legsAnim = 0;//bladeNum

		VectorCopy(boltPoint, te.currentState.pos.trBase);
		VectorCopy(boltAngles, te.currentState.apos.trBase);
		
		if (!te.currentState.apos.trBase[0] && !te.currentState.apos.trBase[1] && !te.currentState.apos.trBase[2])
		{ //don't let it play with no direction
			te.currentState.apos.trBase[1] = 1;
		}

		te.currentState.eventParm = 16; //lots of sparks
		CG_EntityEvent( &te, te.currentState.apos.trBase );
	}
}
...
================
*/
void G_SetOrigin( centity_t *ent, vec3_t origin ) {
	VectorCopy( origin, ent->currentState.pos.trBase );
	ent->currentState.pos.trType = TR_STATIONARY;
	ent->currentState.pos.trTime = 0;
	ent->currentState.pos.trDuration = 0;
	VectorClear( ent->currentState.pos.trDelta );

	VectorCopy( origin, ent->lerpOrigin );
}

static snapshot_t *oldsnap;
static entityState_t myEnts[MAX_ENTITIES_IN_SNAPSHOT];
static centity_t myCents[MAX_ENTITIES_IN_SNAPSHOT];
static int myMaxEnts = 0;

centity_t *G_Spawn()
{
	return &myCents[myMaxEnts++];
}

void G_Dismember( centity_t *ent, /*centity_t *enemy, */vec3_t point, int limbType, float limbRollBase, float limbPitchBase, int deathAnim, qboolean postDeath )
{
	...
	*/
	...
	VectorCopy( newPoint, limb->currentState.pos.trBase );
	//limb->think = LimbThink;
	//limb->touch = LimbTouch;
	limb->currentState.speed = cg.time + Q_irand(8000, 16000);
	//limb->nextthink = cg.time + FRAMETIME;

	//limb->r.svFlags = SVF_USE_CURRENT_ORIGIN;
	//limb->clipmask = MASK_SOLID;
	//limb->r.contents = CONTENTS_TRIGGER;
	//limb->physicsObject = qtrue;
	//VectorSet( limb->currentState.mins, -6.0f, -6.0f, -3.0f );
	//VectorSet( limb->currentState.maxs, 6.0f, 6.0f, 6.0f );

	limb->currentState.g2radius = 200;

	limb->currentState.eType = ET_GENERAL;
	limb->currentState.weapon = G2_MODEL_PART;
	limb->currentState.modelGhoul2 = limbType;
	limb->currentState.modelindex = ent->currentState.number;
	/*if (!ent->client)
	{
		limb->currentState.modelindex = -1;
		limb->currentState.otherEntityNum2 = ent->s.number;
	}*/

	VectorClear(limb->currentState.apos.trDelta);

	/*if (ent->client)
	{
		VectorCopy(ent->client->ps.viewangles, limb->r.currentAngles);
		VectorCopy(ent->client->ps.viewangles, limb->currentState.apos.trBase);
	}
	else
	{*/
		VectorCopy(ent->lerpAngles, limb->lerpAngles);
		VectorCopy(ent->lerpAngles, limb->currentState.apos.trBase);
	//}

	//Set up the ExPhys values for the entity.
	//limb->epGravFactor = 0;
	VectorClear(/*limb->epVelocity*/limb->currentState.pos.trDelta);
	VectorSubtract( point, ent->lerpOrigin, dir );
	VectorNormalize( dir );
	/*if (ent->client)
	{
		VectorCopy(ent->currentState.pos.trDelta, vel);
	}
	else
	{*/
		VectorCopy(ent->currentState.pos.trDelta, vel);
	/*}*/
	VectorMA( vel, 80, dir, /*limb->epVelocity*/limb->currentState.pos.trDelta );

	//add some vertical velocity
	if (limbType == G2_MODELPART_HEAD ||
		limbType == G2_MODELPART_WAIST)
	{
		limb->currentState.pos.trDelta[2] += 10;
	}
	...
	limb->currentState.customRGBA[0] = ent->currentState.customRGBA[0];
	limb->currentState.customRGBA[1] = ent->currentState.customRGBA[1];
	limb->currentState.customRGBA[2] = ent->currentState.customRGBA[2];
	limb->currentState.customRGBA[3] = ent->currentState.customRGBA[3];
...
*/
	...
	if (CG_Cvar_Int( "alwaysGib" ) &&
		cent->ghoul2 &&
		(cent->currentState.eType == ET_BODY || cent->currentState.eFlags & EF_DEAD))
	{ //dismember
		int hitLocUse, i;
		vec3_t boltPoint;
		static int lastTime = 0;
		
		if( cent->currentState.clientNum != lastTime )
		{
			lastTime = cent->currentState.clientNum;
			
			for( hitLocUse = G2_MODELPART_HEAD; hitLocUse <= G2_MODELPART_RLEG; hitLocUse++ )
			{
				if (1/*ent->client*/)
				{
					G_GetDismemberBolt(cent, boltPoint, hitLocUse);
				...
				{
					G_GetDismemberLoc(cent, boltPoint, hitLocUse);
				}
				G_Dismember(cent, /*enemy, */boltPoint, hitLocUse, 90, 0, cgs.clientinfo[cent->currentState.number].legsAnim, qfalse);
			}
			
			CG_Printf("all dismembered.\n");
			
			for( i = 0; i < myMaxEnts; i++ )
			{
				cg.snap->entities[cg.snap->numEntities] = myCents[i].currentState;
				// go backwards
				cg.snap->entities[cg.snap->numEntities].number = MAX_GENTITIES - 1 - i;
				cg_entities[MAX_GENTITIES - 1 - i] = myCents[i];
				cg.snap->numEntities++;
			}
		}
	}
...
static void CG_AddCEntity( centity_t *cent ) {
	...
	int oldNum=0,oldState=0;
	int myps[MAX_POWERUPS],mypw[MAX_POWERUPS];
...
Ghoul2 Insert End
*/
	if( pug_demos_HidePacketEntities.integer & ( 1 << cent->currentState.eType ) ) return;
	...
	case ET_GENERAL:
		...
		break;
	case ET_PLAYER:
		memcpy( myps, cg.predictedPlayerState.powerups, sizeof(int) * MAX_POWERUPS );
		memcpy( mypw, cg.snap->ps.powerups, sizeof(int) * MAX_POWERUPS );
		oldNum = cg.snap->ps.clientNum;
		oldState = cg.predictedPlayerState.pm_type;
		CG_Player( cent );
		if(pug_demospec.integer!=-1) {
			cg.snap->ps.clientNum=oldNum;
			cg.predictedPlayerState.pm_type=oldState;
			memcpy( cg.predictedPlayerState.powerups, myps, sizeof(int) * MAX_POWERUPS );
			memcpy( cg.snap->ps.powerups, mypw, sizeof(int) * MAX_POWERUPS );
		}
		break;
	case ET_ITEM:
		CG_Item( cent );
